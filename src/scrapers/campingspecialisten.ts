...
export async function scrapeCampingspecialisten(url: string): Promise<number> {
  ...
  const price =
    ...
    extractCampingspecialistenCurrentPrice(html) ??
    extractPriceAfterFirstH1(html);

  if (price === null) {
    throw new Error("Kunde inte hitta huvudpriset pa Campingspecialisten-sidan. Satt SCRAPER_DEBUG=true och kor igen for att spara HTML i output/scraper-debug.");
  }

  return price;
}

function extractCampingspecialistenCurrentPrice(html: string): number | null {
