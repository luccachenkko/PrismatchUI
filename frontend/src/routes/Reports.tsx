...
import { EmptyState, ErrorState, KV, Section, Workspace } from "@/components/pm/Workspace";
...
export function ReportsRoute() {
  ...
  return (
    <Workspace
        ...
        <div>
          ...
          {isLoading ? (
            ...
            <EmptyState
              ...
              hint="Kör en prismatchning från dashboarden för att skapa den första rapporten."
            />
          ) : (
      ...
      contextTitle={selected ? `Körning #${selected.id}` : "Välj rapport"}
      context={
        selected ? (
          <ReportContext runId={selected.id} />
        ) : (
          <div className="px-5 py-8 text-[12px] text-muted-foreground">
            Markera en rapport för att se progress och statistik.
          </div>
        )
      }
    />
...
function ReportContext({ runId }: { runId: number }) {
