...
export function Workspace({
  ...
  return (
    <div className="workspace-shell flex h-full min-h-0 flex-1">
      <section className="flex h-full min-w-0 flex-1 flex-col">
        <header className="workspace-header flex h-14 shrink-0 items-center justify-between px-6">
        ...
        {toolbar && (
          <div className="workspace-toolbar flex min-h-12 shrink-0 flex-wrap items-center gap-2 px-5 py-2">
            {toolbar}
          </div>
        )}
      ...
      {context !== undefined && (
        <aside className="workspace-context flex h-full w-[420px] shrink-0 flex-col">
          <div className="workspace-context-title flex h-14 shrink-0 items-center px-5 text-[11px] font-semibold uppercase tracking-wider text-subtle">
...
export function Btn({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`inline-flex items-center justify-center gap-1.5 rounded-md font-medium shadow-sm transition-colors disabled:cursor-not-allowed disabled:opacity-50 ${sizes[size]} ${variants[variant]} ${className}`}
    >
      {children}
    </button>
  );
...
export function TextInput(
  ...
  return (
    <input
      {...rest}
      className={`block w-full rounded-md border border-border bg-surface px-2 py-1.5 text-[12px] text-foreground placeholder:text-subtle focus:border-ring focus:outline-none focus:ring-1 focus:ring-ring ${
        mono ? "font-mono" : ""
      } ${className}`}
    />
  );
